...
export default class DateFormatter { 
    ...
    getISOFormatDate() {
        ...
        return date[2] ? (
            date[2] + '-' + this.addLeadingZero(date[1]) + '-' + this.addLeadingZero(date[0])
        ) : '';
    }
    ...
    getFixedDateString(value) {
        var datePattern = this.datePattern, date = [],
        ...
        var result = date.length === 0 ? value : datePattern.reduce((previous, current) => {
            switch (current) {
            case 'd':
