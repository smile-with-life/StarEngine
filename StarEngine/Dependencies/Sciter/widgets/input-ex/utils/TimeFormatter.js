...
export default class TimeFormatter {
    ...
    initBlocks() {
        for(let pat of this.timePattern) this.blocks.push(2);
    }

    getISOFormatTime() {
        var time = this.time;
        return time[2] ? (
            this.addLeadingZero(time[0]) + ':' + this.addLeadingZero(time[1]) + ':' + this.addLeadingZero(time[2])
        ) : '';
    }
    ...
    getValidatedTime(value) {
        ...
        var timeFormatOptions = this.getTimeFormatOptions();
    ...
    getFixedTimeString(value) {
        var timePattern = this.timePattern, time = [],
