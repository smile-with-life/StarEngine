...
{
public:
    virtual Image Parse(ByteArray& buffer) = 0;
};
...
{
public:
    Image Parse(ByteArray& buffer) override {
        Image img;
        const auto* pFileHeader =
            reinterpret_cast<const BITMAP_FILEHEADER*>(buffer.Data());
        ...
        if (pFileHeader->Signature == 0x4D42 /* 'B''M' */) {
            ...
            img.pixelBit = 32;
            img.colorDepth = 8;
            img.pixelFormat = PIXEL_FORMAT::RGBA8;
